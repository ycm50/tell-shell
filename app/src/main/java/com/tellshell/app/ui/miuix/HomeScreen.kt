            ...
            BoxWithConstraints(
                ...
                Row(
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        val allSelected = uiState.filteredAppList.isNotEmpty() &&
                                uiState.filteredAppList.all { it.packageName in uiState.selectedPackages }
                        val toggleState = remember(allSelected) {
                            if (allSelected) ToggleableState.On else ToggleableState.Off
                        }
                        Checkbox(
                            state = toggleState,
                            onClick = onToggleSelectAllApps
                        )
                        Spacer(Modifier.width(4.dp))
                        Text(
                            text = "全选 (${uiState.filteredAppList.size})"
                        )
                    }
                    Text(
